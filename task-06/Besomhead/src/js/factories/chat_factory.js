...
var chatFactory = (function chatFactoryModule(config) {
  ...
  function updateMessages(event) {
    ...
    if (event.data.path === "/") {
      ...
      chatFactory.appendMessages(DM.getDOMElement(MESSAGES_LIST), userMessages);
      return;
    }

    userMessages[event.data.path.split("/").pop()] = data;
  ...
  ChatFactory.prototype.subscribeOnUpdates = function subscribeOnUpdates() {
    if (config.updates === "refetch") {
      ...
      window.addEventListener("message", updateMessages);
      updatesManager.getDataFromStorage(getExtraPath(MESSAGES_FIELD));
    }
  };
  ...
  function expandChat() {
    ...
    appendMessagesList();
    if (userMessages) {
      showMessages(DM.getDOMElement(MESSAGES_LIST), userMessages);
    }
    appendInputBox();
    if (
      config.userName === "" ||
