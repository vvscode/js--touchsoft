...
function sendBtnOnClick() {
    if(msgInput && msgLog && msgInput.value !== '') {
        ...
        msgInput.value = '';
        setTimeout(function () {
            var reply = {date: getDate(), sender: 'Bot', content: ' Response to the: "' + message.content.toLocaleUpperCase() + '"'};
            appendMessage(reply);
...
function appendMessage(message) {
    ...
    var text = document.createTextNode(message.date + ' ' + message.sender + ': ' + message.content);
    li.appendChild(text);
