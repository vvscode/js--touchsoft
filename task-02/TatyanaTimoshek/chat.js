...
function findId() {
  if (localStorage.getItem('idForTanyaChat') !== null) {
    return localStorage.getItem('idForTanyaChat');
  } else {
    var yourId = generateId();
...
function addMes(time, sender, text) {
  ...
  if (sender === 'Bot' && config.botName !== 'Bot') sender = config.botName;
  if (sender === 'You' && aboutUser.userName !== 'You') {
    sender = aboutUser.userName;
    console.log('ab ' + aboutUser.userName);
    console.log('sen ' + sender);
  }
  var raw = config.showTime ? time + ' ' : ' ';
  h.innerHTML = h.innerHTML + raw + sender + ': ' + text + '<br>';
  scrollDown();
}

function postData(url, requestType, data) {
  if (config.networkFetch) return postDataFetch(url, requestType, data);
  else return postDataXHR(url, requestType, data);
}
