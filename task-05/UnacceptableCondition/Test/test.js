...
tests("DataSource.usersAPI.getUserData should receive valid data",
    function test(assert) {
        ...
        dataSource.usersAPI
            .getUserData(testConfig.testUser.id)
            .then(function getData(data) {
                ...
                assert.equal(
                    typeof data.lastOnline,
                    "number",
                    " lastOnline has valid type"
                );
                assert.equal(
                    typeof data.userName,
                    "string",
                    "userName has valid type"
                );
                assert.equal(
                    typeof data.userId,
                    "string",
                    "userId has valid type"
                );
            })
...
tests("sendMessageToUser should send message to dataBase", function test(
    ...
    var testMessageObject = {
        ...
        read: false,
        title: "message",
        sender: testConfig.testUser.id
    };
    dataSource.usersAPI.sendMessage(testConfig.testUser.id, testMessageObject)
        .then(function sendMess() {
            dataSource.usersAPI
                .getUserData(testConfig.testUser.id)
                .then(function getData(data) {
                    Object.keys(data.messages).map(function getElem(elem) {
                        testData = data.messages[elem].message;
                        return true;
                    });
                    assert.equal(
                        testData,
...
tests("setField should set a field to dataBase", function test(assert) {
    ...
    var testData;
    dataSource.usersAPI
        .setField(testConfig.testUser.id, testField, 99)
...
tests("getField should get a field from dataBase", function test(assert) {
    ...
    var testField = "testField";
    dataSource.usersAPI
        .getField(testConfig.testUser.id, testField)
        .then(function getData(data) {
