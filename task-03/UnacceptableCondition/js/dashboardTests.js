...
tests("dashboardDataSource.oneUserAPI.getUserData should receive valid data", function test(
    ...
    dashboardDataSource.oneUserAPI.getUserData(testUserId).then( function getData (data) {
            ...
            assert.equal(typeof data.settings, "object", " settings has valid type");
        }
    ).then(function () {
        done();
    });
...
tests("setAmountOfNoReadMessage should send amount of noReadMessages to dataBase", function test(
    assert
) {
    var done = assert.async();
    var testData;
...
tests("setField should set a field to dataBase", function test(
    assert
) {
    var done = assert.async();
...
tests("getField should get a field from dataBase", function test(
    assert
) {
    var done = assert.async();
    var testField = "testField";
...
tests("createUserElement should valid div element", function test(
    assert
) {
    var testData = userListManager.createUserElement(testUserId, true);
    assert.equal(testData.firstChild.tagName, "DIV", " element has valid tag name");
    assert.equal(testData.lastChild.tagName, "DIV", " element has valid tag name");
    assert.ok(testData.firstChild.classList.contains(userListManagerConfig.USER_ID_ELEMENT_CSS_CLASS), " element has valid class name");
    assert.ok(testData.lastChild.classList.contains(userListManagerConfig.USER_INDICATOR_CSS_CLASS_ONLINE), " element has valid class name");
    assert.ok(testData.classList.contains(userListManagerConfig.USER_ELEMENT_CSS_CLASS), " element has valid class name");

});
tests("setUserList should set new uList", function test(
    assert
) {
...
tests("getUserFromUserListById should return key user on uList by userId", function test(
    assert
) {
    var newList = {testKey: {userId: testUserId}};
    userListManager.setUserList(newList);
    assert.equal( userListManager.getUserFromUserListById(testUserId), "testKey", "getUserFromUserListById true");
});
tests("userIsOnline should return true if user is online", function test(
    ...
    assert.equal( userListManager.userIsOnline(date.getTime()), true, "getUserFromUserListById true");
    assert.equal( userListManager.userIsOnline(date.getTime() - userListManagerConfig.ONLINE_INTERVAL - negativeConst), false, "userIsOnline true");
});
tests("filterByName should filter name by str", function test(
    assert
) {
    var newList = [
        {
...
tests("sortUsersByField should sort by field", function test(
    assert
) {
    var newList = [
        {
...
tests("createMessageElement should create valid message element", function test(
    ...
    var messageElement = chatManager.createMessageElement(message, messageDate, userName, isRead);
    assert.equal( messageElement.tagName,"DIV", "tagName true");
    assert.equal( messageElement.innerText,innerHtml, "message true");
    assert.ok(messageElement.classList.contains(chatManagerConfig.CSS_USER_NOT_READ_MESSAGES), "class true");
});
tests("addMessageToMessageList should add message", function test(
    assert
) {
    var message = "test";
    chatManager.messageList = [];
    chatManager.addMessageToMessageList(message);
    assert.equal( chatManager.messageList.length,1, "addMessageToMessageList true");

});
tests("updateMessageList should mark no read messages from admin", function test(
    assert
) {
    var messageObj1 = chatManager.createMessageObject("test1", "testDate", chatManagerConfig.DEFAULT_ADMIN_NAME, true);
    var messageObj2 = chatManager.createMessageObject("test2", "testDate", chatManagerConfig.DEFAULT_ADMIN_NAME, true);
